 ...
 * 
 * Generates a blocking key based on the first three characters of a provoded label column
 * 
 */
public class BlockingKeyByLabelTokens extends
	...
	public BlockingKeyByLabelTokens(LinkedList<MatchableTableColumn> labels, int noCharacters){
		this.labels = labels;
					...
					if(record.hasValue(column)){
						...
						if (value.contains(" ")){
							...
							for(String val: values){
								keyValue = keyValue + val.substring(0, Math.min(val.length(), noCharacters));
							}
						}
						else{
							keyValue = value.substring(0, Math.min(value.length(), noCharacters));
						}
