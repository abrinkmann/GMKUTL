 ...
 * 
 * Generates a blocking key based on the year of any date.
 * (Dates are parsed using the typeConverter.)
 * 
 */
public class BlockingKeyByYear extends
	...
	public BlockingKeyByYear(LinkedList<MatchableTableColumn> labels){
		this.labels = labels;
			...
			DataIterator<Pair<String, MatchableTableRow>> resultCollector) {
				for(MatchableTableColumn column : labels){
					if(record.hasValue(column)){
						...
						LocalDateTime date = (LocalDateTime) tc.typeValue(value, DataType.date, null);
						if(date != null){
							resultCollector.next(new Pair<>(Integer.toString(date.getYear()), record));
						}
					}
				}
	}
