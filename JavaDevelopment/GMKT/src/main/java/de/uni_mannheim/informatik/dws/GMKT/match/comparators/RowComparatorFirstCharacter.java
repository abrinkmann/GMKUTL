 ...
 * 
 */
public class RowComparatorFirstCharacter extends StringComparator {

	public RowComparatorFirstCharacter() {
	}
	...
	public double compare(MatchableTableRow record1, MatchableTableRow record2,
		...
		if (record1.hasValue(schemaCorrespondence.getFirstRecord())) {
			...
			if (s1.length() > 0) {
				prepS1 = s1.toLowerCase().substring(0, 1);
		...
		if (record2.hasValue(schemaCorrespondence.getSecondRecord())) {
			...
			if (s2.length() > 0) {
				prepS2 = s2.toLowerCase().substring(0, 1);
